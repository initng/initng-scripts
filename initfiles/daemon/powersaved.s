...
setup()
{
	iregister -s "daemon/powersaved/prepare" service
	iregister -s "daemon/powersaved" daemon

	iset -s "daemon/powersaved/prepare" need = "system/bootmisc daemon/acpid"
	iset -s "daemon/powersaved" need = "system/bootmisc system/modules daemon/dbus daemon/hald daemon/powersaved/prepare daemon/acpid"
	iset -s "daemon/powersaved" pid_file = "/var/run/powersaved.pid"
	iset -s "daemon/powersaved" forks

	iexec -s "daemon/powersaved/prepare" start = prepare_start
	iexec -s "daemon/powersaved" daemon = "@/usr/sbin/powersaved@ -d -f /var/run/acpid.socket"

	idone -s "daemon/powersaved/prepare"
	idone -s "daemon/powersaved"
}
