...
setup()
{
	iregister -s "daemon/xfs/chkfontpath" service
	iregister -s "daemon/xfs" daemon

	iset -s "daemon/xfs/chkfontpath" need = "system/bootmisc"
#ifd gentoo
	iset -s "daemon/xfs" need = "system/bootmisc"
#elsed
	iset -s "daemon/xfs" need = "system/bootmisc daemon/xfs/chkfontpath"
#endd

	iexec -s "daemon/xfs/chkfontpath" start = chkfontpath_start
#ifd debian
	iexec -s "daemon/xfs" daemon = "@/usr/X11R6/bin/xfs@ -droppriv -nodaemon"
#elsed gentoo
	iexec -s "daemon/xfs" daemon = "/usr/bin/xfs -nodaemon -config /etc/X11/fs/config -droppriv -user xfs -port ${XFS_PORT}"
#elsed
	iexec -s "daemon/xfs" daemon = "@/usr/bin/xfs@ -droppriv -nodaemon"
#endd

	idone -s "daemon/xfs/chkfontpath"
	idone -s "daemon/xfs"
}
