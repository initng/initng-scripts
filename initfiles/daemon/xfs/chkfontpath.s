...
start() {
	...
	if [ -x @/usr/sbin/chkfontpath@ ]; then
		...
		for dir in `@/usr/sbin/chkfontpath@ --list |
		            @sed@ -e '/^Current/d;s#^[0-9]*: ##g;s#^/.*:[a-z]*$##g;/^[[:space:]]*$/d' |
                            @sort@ -u`; do
			if [ -d "$dir" ]; then
				cd "$dir"
				...
				if [ ! -e fonts.dir -o -n "`@find@ . -maxdepth 1 -type f -cnewer fonts.dir -not -name 'fonts.cache*'`" ]; then
					@rm@ -f fonts.dir
