...
start()
{
	...
	then
		...
		do
			...
			[ X"${QUIET_PERVC}" != X1 ] && echo -n "/dev/tty${vc}, "

			# eventually find an associated SFM
			eval sfm=\${SCREEN_FONT_MAP_vc${vc}}
			[ "${sfm}" ] && sfm="-u ${sfm}"

			@/usr/bin/consolechars@ --tty=/dev/tty${vc} ${SETFONT_OPT} -f ${font} ${sfm}
		done
	fi
	...
	then
		...
		do
			...
			[ "${QUIET_PERVC}" != "1" ] && echo -n "/dev/tty${vc} (${acm}), "
			eval "@/usr/bin/charset@ --tty='/dev/tty${vc}' G0 '${acm}'"
		done
	fi
