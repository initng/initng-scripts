...
static const char *probe_distribution(void)
{
	...
	if (is_file("/etc/altlinux-release"))
		return "altlinux";
	if (is_file("/etc/mandriva-release"))
		return "mandriva";
...
static char **str_split(const char *in, char delim)
{
	...
	/* it doesn't matter if we allocate a bit more mem than required,
	 * eg for :: */
...
static char *do_exec(char *data, int length)
{
	...
	if ((pid = fork()) == 0) {
		...
		if (TEMP_FAILURE_RETRY(write(pipe_fork[1], data, length)) !=
		    length ||
...
static inline int match_distro(const char *in, const char *distro)
{
	...
	while (1) {
		...
		if ((tmp[-1] != ' ' && tmp[-1] != '\t')
		    || (tmp[len] != ' ' && tmp[len] != '\t'
			&& tmp[len] != '\n'))
			tmp++;	/* this will make strstr not find the same
				 * part again */
		else
			return 1;
	}
